...
Free Cone and trash images from Cartoonstock.com
Scoops made by me */
...
float stack = 0;  // sets stack value to 0
...
void setup(){
  ...
  caption = loadFont("AvenirNext-Heavy.vlw"); // calls and loads the data for font
  ...
  int index2 = int(random(0, imgTrash.length)); // program picks random image from the arra
  trash[t] = new Trash (imgTrash[index2],random(0,width),random(-300,-100)); } // assigns data to class constructor
...
void draw(){
 ...
 if (gameScreen == 0){ // always begins at initial screen bc gameScreen is set to zero at start
   ...
   playGameScreen();
 } else if (gameScreen == 2) { //when game screen is 2, game over screen pops u
   gameOverScreen();
 } 
...
public void mousePressed() {
  ...
  if (gameScreen == 0) { // gameScreen is added whenever called
    startGame();
  }
  if (gameScreen == 2) { // when the mouse is pressed go from 0 to startgame() which is one
    gameOver();
  }
    if (gameScreen == 3) { // with the screen at one, when the player loses the screen goes to two which sends them to the gameover screen
...
void playGameScreen(){ // where all the magic happens
